...
const isLikelyFilePath = (candidate: string, cwd: string): boolean => {
  // Skip URLs
  ...
  if (candidate.includes(": ") && !candidate.endsWith(".nix")) {
    return false; // Likely "error: /path/to/thing"
  }

  // Skip /usr/bin/* and /bin/* (system paths, not project files)
  if (candidate.startsWith("/usr/") || candidate.startsWith("/bin/")) {
    return false;
  }

  // Prefer paths that exist
  const normalized = normalizeReferencePath(candidate, cwd);
  if (normalized && existsSync(normalized)) {
    return true;
  }

  // Allow if it looks like project file (relative-ish)
  if (candidate.startsWith("./") || candidate.startsWith("../")) {
    return true;
  }

  // Skeptical of bare /paths unless they exist
  return false;
};

const extractFileReferencesFromText = (text: string, cwd: string): string[] => {
  const refs: string[] = [];

  // 1. <file name="..."> tags (explicit, always include)
  for (const match of text.matchAll(FILE_TAG_REGEX)) {
    refs.push(match[1]);
  }

  // 2. file:// URLs (explicit, always include)
  for (const match of text.matchAll(FILE_URL_REGEX)) {
    refs.push(match[0]);
  }

  // 3. Paths (filter aggressively to avoid false positives)
  for (const match of text.matchAll(PATH_REGEX)) {
    const candidate = match[1];
    if (isLikelyFilePath(candidate, cwd)) {
      refs.push(candidate);
    }
  }
...
const extractFileReferencesFromContent = (
  ...
  if (typeof content === "string") {
    return extractFileReferencesFromText(content, cwd);
  }
  ...
  for (const part of content) {
    ...
    if (block.type === "text" && typeof block.text === "string") {
      refs.push(...extractFileReferencesFromText(block.text, cwd));
    }
...
const extractFileReferencesFromEntry = (
  ...
  if (entry.type === "message") {
    return extractFileReferencesFromContent(entry.message.content, cwd);
  }

  if (entry.type === "custom_message") {
    return extractFileReferencesFromContent(entry.content, cwd);
  }
...
const collectRecentFileReferences = (
  ...
  for (let i = entries.length - 1; i >= 0 && results.length < limit; i -= 1) {
    const refs = extractFileReferencesFromEntry(entries[i], cwd);
...
const openDiff = async (
  ...
  // Cleanup temp directory after VS Code loads the files (async, don't block)
  setTimeout(async () => {
    try {
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerShortcut("ctrl+shift+r", {
    description: "Quick Look the latest file",
    handler: async (ctx) => {
      const entries = ctx.sessionManager.getBranch();
      const latest = findLatestFileReference(entries, ctx.cwd);
