...
function sumSessionUsage(ctx: ExtensionCommandContext): {
  ...
  for (const entry of ctx.sessionManager.getEntries()) {
    ...
    cacheRead += Number(usage.cacheRead ?? usage.cacheReadInputTokens ?? 0) || 0;
    cacheWrite += Number(usage.cacheWrite ?? usage.cacheCreationInputTokens ?? 0) || 0;
  ...
  private rebuild(width: number): void {
    ...
    if (!this.data.usage) {
      ...
      if (u.percent >= 95) {
        ...
        lines.push(this.theme.fg("warning", "⚠️  Warning: Context is 80%+ full"));
      }
    }
...
export default function contextExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("context", {
    ...
    handler: async (_args, ctx: ExtensionCommandContext) => {
      ...
      // Measure actual tool schema size including parameters and prompt snippets.
      ...
      for (const name of activeToolNames) {
        ...
        if (!info) continue;
        const schema = JSON.stringify(info.parameters ?? {});
        const prompt = info.promptSnippet ?? "";
        const guidelines = (info.promptGuidelines ?? []).join("\n");
        const blob = `${info.name}\n${info.description}\n${schema}\n${prompt}\n${guidelines}`;
