...
interface CommandHooks {
	onManualModeChanged?: (manualMode: boolean) => void;
	onEvent?: (type: string, payload?: Record<string, unknown>) => void;
	onBenchmarkStatus?: (ctx: ExtensionCommandContext, state: DcpState) => void;
}
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
Commands:
  ...
  /dcp benchmark    — Show benchmark logging status`;
...
function handleContext(ctx: ExtensionCommandContext, state: DcpState): void {
	...
	lines.push(
		`  Compression blocks: ${state.compressionBlocks.filter((b) => b.active).length}`,
	);
	lines.push(`  Tokens saved (estimated): ${fmt(state.tokensSaved)}`);
	lines.push(
		`  Tokens replaced by compression: ${fmt(state.tokensReplacedByCompression)}`,
	);
...
function handleManual(
	...
	if (subArg === "on") {
		...
		hooks?.onManualModeChanged?.(true);
	} else if (subArg === "off") {
		state.manualMode = false;
		ctx.ui.notify(
			"Manual mode: off\nAutonomous compression is enabled.",
			"info",
		);
		hooks?.onManualModeChanged?.(false);
	} else {
...
function handleDecompress(
	...
	if (nArg === undefined) {
		...
		if (!block.active) {
			ctx.ui.notify(
				`Compression block b${id} is already decompressed.`,
				"info",
			);
			return;
		}
...
function handleBenchmark(
	...
	hooks?.onBenchmarkStatus?.(ctx, state);
}
		...
		getArgumentCompletions(prefix: string): AutocompleteItem[] | null {
			const subcommands: AutocompleteItem[] = [
				...
				{ value: "manual", label: "manual", description: "Toggle manual mode" },
				{
					value: "decompress",
					label: "decompress",
					description: "List or restore compression blocks",
				},
				{
					value: "compress",
					label: "compress",
					description: "Trigger LLM compression",
				},
				{ value: "help", label: "help", description: "Show help" },
				{
					value: "benchmark",
					label: "benchmark",
					description: "Show benchmark logging status",
				},
			];
		...
		async handler(args: string, ctx: ExtensionCommandContext): Promise<void> {
			...
			switch (sub) {
				...
				case "sweep": {
					...
					hooks?.onEvent?.("sweep", { count: n });
					break;
				}

				case "manual":
					handleManual(ctx, state, parts[1], hooks);
					break;

				case "decompress":
					handleDecompress(ctx, state, parts[1]);
					hooks?.onEvent?.("decompress", { arg: parts[1] ?? null });
					break;

				case "compress":
					await handleCompress(pi, ctx);
					hooks?.onEvent?.("compress_trigger", {});
					break;

				case "benchmark":
					handleBenchmark(ctx, state, hooks);
					break;
