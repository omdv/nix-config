...
function normalizeMessages(input: unknown[]): DcpMessage[] {
	return input.filter(isRecord) as DcpMessage[];
}
	...
	for (const block of activeBlocks) {
		...
		while (lo > 0) {
			...
			const hasMatchingToolCalls = prevContent.some(
				(block: any) =>
					block.type === "toolCall" && toolCallIdsInRange.has(block.id),
			);
...
function repairOrphanedToolPairs(messages: any[]): void {
	...
	for (let i = messages.length - 1; i >= 0; i--) {
		...
		if (
			...
			!assistantToolCallIds.has(msg.toolCallId)
		) {
			messages.splice(i, 1);
		}
	}
...
export function applyPruning(
	...
	// Performance cap: process only the most recent window of messages.
	const cap = Math.max(1, config.compress.maxMessagesScanned ?? 800);
	const scoped =
		normalized.length > cap
	...
	const msgs: any[] = scoped.map((m: any) => {
		const clone = { ...m };
...
export function getNudgeType(
	...
	if (
		contextPercent > minContextPercent &&
